...
{ 
  assert(P_rev_1:forall(n:nat)(b:B),repeat b n ++[b] = b::repeat b n). {
    intros. induction n. 
      -simpl. reflexivity.
      -simpl. rewrite IHn. reflexivity. }
  assert(P_rev_2:forall(n:nat)(b:B), rev(repeat b n)= repeat b n). {
    intros. induction n.
       -reflexivity.
       -simpl. rewrite IHn. rewrite P_rev_1. reflexivity. }
intros. unfold AddL. rewrite P_rev_2. induction n.
  -simpl. reflexivity.
  -simpl. rewrite IHn. rewrite P_rev_1. reflexivity. }
intros. simpl. rewrite H. rewrite P. reflexivity.
Qed.

Lemma ALU_minus_one (x y : list B) : 
  length x = length y -> ALU x y I I I O I O = repeat I (length x).
Proof. intros H. simpl. rewrite H.
...
-intros. destruct y.
  --simpl. reflexivity.
  --simpl. inversion H. destruct a,b.
      ---simpl. rewrite IHx. reflexivity. exact H1.
      ---simpl. rewrite IHx. reflexivity. exact H1.
      ---simpl. rewrite IHx. reflexivity. exact H1.
      ---simpl. rewrite IHx. reflexivity. exact H1.
Qed.
...
Proof.
  simpl. intros [lx[ly n0]]. rewrite lx,ly.
  assert (H_NotL : forall (n : nat), NotL (repeat O n) = repeat I n).
  {
    induction n1.
    - simpl. reflexivity.
